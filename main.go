...
or updated comments added in later versions of lyricvid.`,
	...
	initCmd := &cobra.Command{
		Use:   "init <audio> [project]",
		Short: "Initialise a song project with folder, config, and empty LRC file",
		Long: `Creates the project image folder, a YAML config file pre-filled with
fade-in/out settings, and an empty .lrc file (if one doesn't already exist).

  project  name of the image sub-folder and config file stem (default: images)`,
		Args: cobra.RangeArgs(1, 2),
		RunE: runInit,
	}
...
func runInit(_ *cobra.Command, args []string) error {
	...
	songDir, songStem := audioStem(audioPath)
	...
	imageDir := filepath.Join(songDir, project)
	if err := os.MkdirAll(imageDir, 0755); err != nil {
		return fmt.Errorf("creating image folder %q: %w", imageDir, err)
	}
	fmt.Printf("Image folder: %s\n", imageDir)

	// 2. Write the project config file.
	configPath := filepath.Join(songDir, project+".yml")
	if _, err := os.Stat(configPath); err == nil {
		...
		yml = strings.ReplaceAll(yml, `# fade-in-title: ""`, `fade-in-title: "`+songStem+`"`)
		yml = strings.ReplaceAll(yml, "# fade-out-seconds: 0", "fade-out-seconds: 5")
		yml = strings.ReplaceAll(yml, `# fade-out-title: ""`, `fade-out-title: "made with lyricvid"`)
		yml = strings.ReplaceAll(yml, `# fade-out-font-color: ""`, `fade-out-font-color: "#FF0000"`)
		if err := os.WriteFile(configPath, []byte(yml), 0644); err != nil {
			return fmt.Errorf("writing config %q: %w", configPath, err)
		}
		fmt.Printf("Config written: %s\n", configPath)
	}

	// 3. Create an empty LRC file if one doesn't exist.
	lrcPath := filepath.Join(songDir, songStem+".lrc")
	if _, err := os.Stat(lrcPath); err == nil {
		fmt.Printf("LRC already exists, skipping: %s\n", lrcPath)
	} else {
		if err := os.WriteFile(lrcPath, []byte{}, 0644); err != nil {
			return fmt.Errorf("creating LRC file %q: %w", lrcPath, err)
		}
		fmt.Printf("LRC created: %s\n", lrcPath)
	}

	// Print next-steps guide.
	sep := strings.Repeat("─", 60)
	h := func(s string) { fmt.Printf("\n  %s\n", s) }
	step := func(n, title string) { fmt.Printf("\n  %s  %s\n", n, title) }
	sub := func(s string) { fmt.Printf("      %s\n", s) }
	cmd := func(s string) { fmt.Printf("      \033[96m$ %s\033[0m\n", s) }
	fmt.Printf("\n%s\n", sep)
	fmt.Println("  Project initialised — what to do next")
	fmt.Printf("%s\n", sep)

	step("1.", "Add lyrics to your LRC file")
	sub("Open and fill in timestamped lyrics:")
	sub(lrcPath)
	sub("Each line follows the format:  [mm:ss.xx] Lyric text")
	sub("Use a tool like LRCMaker or edit by hand.")

	step("2.", "Generate background images  (optional)")
	sub("Make sure you have a Gemini API key stored:")
	cmd("lyricvid set-gemini <your-api-key>")
	sub("Then generate images from your lyrics:")
	cmd("lyricvid image-gen \"" + audioPath + "\" " + project)

	h("    — or drop your own images into:")
	sub(imageDir)
	sub("Supported formats: .jpg  .jpeg  .png  .webp")

	step("3.", "Customise the project config")
	sub("Open and tweak " + configPath + " to adjust:")
	sub("  • fade-in-title / fade-out-title — change the intro/outro text")
	sub("  • fade-out-font-color            — red (#FF0000) by default")
	sub("  • font-color / highlight-color   — lyric colours")
	sub("  • quality                        — 480p | 720p | 1080p | 1440p")
	sub("  • aspect-ratio                   — e.g. 16:9  4:3  9:16")
	sub("  • drift                          — pan/zoom animation (zoom-in, left, …)")

	step("4.", "Render the video")
	cmd("lyricvid \"" + audioPath + "\" --config \"" + configPath + "\"")
	sub("Output will be saved alongside the audio file as a .mp4.")
